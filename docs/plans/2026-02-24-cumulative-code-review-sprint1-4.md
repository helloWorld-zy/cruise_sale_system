# Cumulative Code Review Prompt: Sprint 1 - 4

> **用途**：供大模型在单次会话中完整审查 Sprint 1 至 Sprint 4 的代码实现。
> **核心挑战**：代码库已包含 Sprint 1-4 的全部代码，后续 Sprint 引入的文件和改动不应导致前序 Sprint 的 Review 误判。

---

## 统一角色设定

**Role**: 你是一个极其严格的资深架构师（Senior Staff Engineer）、安全审计专家及资深 QA。你的目标是对 Sprint 1 至 Sprint 4 的代码进行最严苛的逐 Sprint 累进审查（Cumulative Code Review）。
...
**Sprint 4 起新增的重点审查维度**（基于前序 Sprint 的 Review 反馈）:
4. **前端实联验证**: 所有前端页面必须通过 API 与后端通信，禁止存在仅含静态/硬编码数据的"空壳页面"。每个页面必须包含完整数据流（API 调用 → 响应处理 → 数据渲染 → loading/error/empty 状态处理）。
5. **测试质量审计**: 禁止为达成覆盖率而编写无业务含义的测试。每条测试必须包含有意义的业务断言，禁止"伪覆盖"手段（`istanbul ignore`、`//nolint`、空函数体、仅 import 不调用、删除/注释失败测试等）。
...
你面对的代码库已经包含了 Sprint 1、Sprint 2、Sprint 3、Sprint 4 的全部代码。你**必须按顺序**从 Sprint 1 开始审查。审查每个 Sprint 时，遵循以下规则：
...
- 后续 Sprint 可能对前序 Sprint 创建的文件进行了**扩展**（如 Sprint 2 向 `router.go` 添加新路由、Sprint 4 向 `booking_handler.go` 添加支付状态变更逻辑）。
- 审查 Sprint N 时，如果发现某文件中包含 Sprint N+1、N+2 或 N+3 引入的代码，**这不是违规**。
- **判断方法**：参照「后续 Sprint 已知变更清单」（见下方每个 Sprint 审查区块），如果变更属于清单中列出的内容，则**忽略该变更，不计入 Sprint N 的审查结论**。

#### 规则 3：不得以后续 Sprint 的标准反向要求前序 Sprint
- 例如：Sprint 4 引入了 `payment_service.go`，Sprint 3 的代码中自然不会有支付相关的逻辑。不得因"Sprint 3 没有支付功能"而扣分。
- 又如：Sprint 3 引入了 `booking_service.go`，Sprint 1 的代码中自然不会有预订相关的逻辑。不得因此认为 Sprint 1 缺少预订功能。
...
| `docs/plans/2026-02-22-sprint04.md` | Sprint 4 完整实现计划（Part A-E） |
...
步骤 8：执行 Sprint 4 审查（参照下方 Sprint 4 审查区块）
步骤 9：输出 Sprint 4 审查结论
步骤 10：输出汇总报告
```
...
以下变更由 Sprint 2、Sprint 3 或 Sprint 4 引入，审查 Sprint 1 时**必须忽略，不得标记为问题**：
...
| `backend/internal/domain/payment.go` | Sprint 4 | 支付模型，Sprint 4 新增 |
| `backend/internal/domain/refund.go` | Sprint 4 | 退款模型，Sprint 4 新增 |
| `backend/internal/service/payment_service.go` | Sprint 4 | 支付服务，Sprint 4 新增 |
| `backend/internal/service/refund_service.go` | Sprint 4 | 退款服务，Sprint 4 新增 |
| `backend/internal/service/notify_service.go` | Sprint 4 | 通知服务，Sprint 4 新增 |
| `backend/internal/service/analytics_service.go` | Sprint 4 | 统计服务，Sprint 4 新增 |
| `backend/internal/handler/payment_handler.go` | Sprint 4 | 支付 Handler，Sprint 4 新增 |
| `backend/internal/handler/refund_handler.go` | Sprint 4 | 退款 Handler，Sprint 4 新增 |
| `backend/internal/handler/analytics_handler.go` | Sprint 4 | 统计 Handler，Sprint 4 新增 |
| `backend/internal/pkg/payment/payment_gateway.go` | Sprint 4 | 支付网关适配器，Sprint 4 新增 |
| `backend/migrations/000004_*.sql` | Sprint 4 | Sprint 4 迁移文件 |
| `frontend/admin/pages/dashboard/**` | Sprint 4 | 管理后台数据看板，Sprint 4 新增 |
| `frontend/admin/pages/finance/**` | Sprint 4 | 管理后台财务页面，Sprint 4 新增 |
| `frontend/admin/components/StatCard.vue` | Sprint 4 | 统计卡片组件，Sprint 4 新增 |
| `frontend/web/pages/orders/**` | Sprint 4 | 前台订单详情，Sprint 4 新增 |
| `frontend/web/pages/pay/**` | Sprint 4 | 前台支付页面，Sprint 4 新增 |
| `frontend/web/components/PayButton.vue` | Sprint 4 | 支付按钮组件，Sprint 4 新增 |
| `frontend/miniapp/pages/pay/**` | Sprint 4 | 小程序支付页面，Sprint 4 新增 |
| `backend/internal/router/router.go` | Sprint 2/3/4 | 后续 Sprint 可能向此文件添加了新的路由注册，这是合法的扩展 |
| `.github/workflows/ci.yml` | Sprint 2/3/4 | 后续 Sprint 可能添加了新的 CI job，这是合法的扩展 |
...
以下变更由 Sprint 3 或 Sprint 4 引入，审查 Sprint 2 时**必须忽略，不得标记为问题**：
...
| `frontend/web/components/PayButton.vue` | Sprint 4 | 支付按钮组件，Sprint 4 新增 |
| `frontend/miniapp/pages/pay/**` | Sprint 4 | 小程序支付页面，Sprint 4 新增 |
| `backend/internal/router/router.go` | Sprint 3/4 | Sprint 3/4 可能向此文件添加了预订/支付相关路由，这是合法的扩展 |
| `.github/workflows/ci.yml` | Sprint 3/4 | Sprint 3/4 可能添加了新的 CI job，这是合法的扩展 |
...
### Sprint 3 审查时需忽略的后续 Sprint 变更

以下变更由 Sprint 4 引入，审查 Sprint 3 时**必须忽略，不得标记为问题**：
...
| `frontend/web/components/PayButton.vue` | Sprint 4 | 支付按钮组件，Sprint 4 新增 |
| `frontend/miniapp/pages/pay/**` | Sprint 4 | 小程序支付页面，Sprint 4 新增 |
| `backend/internal/handler/booking_handler.go` | Sprint 4 | Sprint 4 可能向此文件添加了支付状态变更逻辑，这是合法的扩展 |
| `backend/internal/router/router.go` | Sprint 4 | Sprint 4 可能向此文件添加了支付/退款/统计相关路由，这是合法的扩展 |
| `.github/workflows/ci.yml` | Sprint 4 | Sprint 4 可能添加了新的 CI job，这是合法的扩展 |

---

## Sprint 4 审查区块（订单支付、通知与统计）

### Sprint 4 计划交付物（审查范围）

参照 `docs/plans/2026-02-22-sprint04.md`，Sprint 4 包含 5 个 Part（A-E），11 个 Task：

- **Part A (Task 1-6)**: 后端 — Payment/Refund Domain 模型、迁移（`000004_payment_notify`）、Payment Service（微信/支付宝适配）、Payment Callback + Order Status、Refund Service/Handler、Notification Service
- **Part B (Task 7)**: 后端 — Analytics Service/Handler（数据看板）
- **Part C (Task 8)**: Admin — Dashboard 数据看板 + Finance 财务页面
- **Part D (Task 9-10)**: Web/Miniapp — 支付页面 + 订单详情
- **Part E (Task 11)**: CI 覆盖更新
...
- [ ] **Part 完整性**: Sprint 4 计划中的 5 个 Part、11 个 Task 是否全部完成？根据 `docs/plans/2026-02-22-sprint04.md` 及 `docs/plans/2026-02-22-master-roadmap.md` 中 Sprint 4 的内容逐一核对。
- [ ] **TDD 合规**: 每个 Task 是否都有对应的测试文件？测试是否覆盖了关键逻辑路径？
- [ ] **前端实联验证（重点）**: 逐页检查 Sprint 4 新增的所有前端页面（Dashboard 看板、Finance 财务页、订单详情、支付页面等），确认每个页面都通过 API 调用后端获取真实数据。具体检查：
  - 每个页面是否调用了对应的后端 API endpoint？
  - 是否正确处理了 loading 加载态、error 错误态、empty 空数据态？
  - 用户操作（如点击支付、提交退款）是否触发了对应的后端 API 调用？
  - 是否存在仅有 UI 骨架但无 API 调用的"空壳页面"？**发现即标记为阻塞性问题**。
  - 是否存在硬编码的假数据或静态展示数据？**发现即标记为阻塞性问题**。
- [ ] **测试质量审计（重点）**: 逐一检查测试用例的业务含义。具体检查：
  - 前端组件测试是否验证了：(1) 正确调用后端 API 或 store action；(2) 将返回数据正确渲染到 DOM；(3) 用户交互触发预期行为？
  - 后端测试是否验证了：(1) handler 接收参数并调用正确 service 方法；(2) service 层业务逻辑对各输入组合返回正确结果；(3) 错误场景返回预期错误码与消息？
  - 是否存在仅断言 `true === true`、仅检查组件能渲染而不验证内容、仅 import 文件而不调用逻辑的"假测试"？**发现即标记为阻塞性问题**。
  - 是否存在 `/* istanbul ignore */` 或 `//nolint` 标记跳过未测代码？**发现即标记为阻塞性问题**。
  - 是否存在空函数体或 noop 实现替代真实逻辑后声称测试通过？**发现即标记为阻塞性问题**。
  - 是否存在通过注释/删除失败测试来提升通过率的行为？**发现即标记为阻塞性问题**。

### Sprint 4 审查时无需忽略的后续变更

Sprint 4 是本次审查的最后一个 Sprint。当前代码库中不应存在 Sprint 5 及以后的代码。如果发现了 Sprint 5+（智能推荐、价格日历、舱位对比等）的代码：
- **标记为异常**：代码库中不应包含超出 Sprint 4 范围的实现。
...
在完成四个 Sprint 的独立审查后，还需进行以下全局检查：

### 跨 Sprint 一致性检查

- [ ] **Domain 模型命名一致性**: Sprint 1 的 `CruiseCompany`/`Cruise`/`CabinType` 与 Sprint 2 的 `Route`/`Voyage`/`CabinSKU`、Sprint 3 的 `User`/`Booking`、Sprint 4 的 `Payment`/`Refund` 之间的关联（外键引用）是否正确？
- [ ] **Repository 接口一致性**: 各 Sprint 的 Repository 是否都遵循了统一的接口模式（`domain/repository.go` 中的接口 vs 具体实现）？
- [ ] **Router 注册完整性**: `router.go` 是否正确注册了 Sprint 1-4 所有需要的路由，且没有遗漏？
- [ ] **迁移链完整性**: `000001` -> `000002` -> `000003` -> `000004` 四个迁移文件是否可以按顺序正确执行？是否存在外键引用的先后顺序问题？
- [ ] **共享类型同步**: `frontend/shared/types/` 中的类型定义是否覆盖了 Sprint 1-4 所有需要的后端数据结构？
- [ ] **CI 管道覆盖**: CI 是否能运行 Sprint 1-4 的所有测试（backend + admin + web + miniapp）？
...
- [ ] **支付安全**: 支付金额是否在后端重新计算（不信任前端传入的金额）？支付回调签名验证是否严格？

### 全局前端实联检查（Sprint 4 起新增）

- [ ] **空壳页面扫描**: 遍历 `frontend/admin/pages/`、`frontend/web/pages/`、`frontend/miniapp/pages/` 下所有 `.vue` 文件，检查是否存在未调用任何 API 的页面组件。
- [ ] **API 调用完整性**: 前端 composables/store 中定义的 API 调用函数，是否都在对应页面中被实际使用？是否存在定义了但从未调用的"死代码"？
- [ ] **错误处理一致性**: 所有前端页面的 API 调用是否都有统一的错误处理（try/catch 或 error ref），而非静默吞掉错误？

### 全局测试质量检查（Sprint 4 起新增）

- [ ] **测试断言质量**: 随机抽样检查至少 10 个测试文件，确认每个测试用例至少包含一个有意义的业务断言（非 `toBeTruthy()`/`toBeDefined()` 之类的弱断言）。
- [ ] **Mock 合理性**: 前端测试中的 API mock 返回值是否与后端实际 API 响应结构一致？是否存在 mock 了一个与真实 API 完全不同的数据结构？
- [ ] **覆盖率真实性**: 运行测试覆盖率报告后，对未覆盖的行进行人工审查，确认是否因代码本身不可达（合理）还是因为偷懒没写测试（不合理）。
...
### 前端实联审查结果（Sprint 4 必填）
| 页面 | API Endpoint | 是否实联 | loading/error/empty 处理 | 备注 |
|------|-------------|---------|------------------------|------|
| dashboard/index.vue | GET /api/analytics/dashboard | 是/否 | 是/否 | ... |

### 测试质量审查结果（Sprint 4 必填）
| 测试文件 | 业务断言数 | 弱断言数 | 伪覆盖标记数 | 判定 |
|---------|-----------|---------|------------|------|
| xxx.test.ts | 5 | 0 | 0 | 通过 |
...
> 4. **前端页面是否真的通过 API 与后端通信？** — 检查是否存在空壳页面或硬编码假数据。仅有 UI 骨架但无 API 调用的页面必须标记为阻塞性问题。
> 5. **测试是否包含有意义的业务断言？** — 检查是否存在伪覆盖手段。仅为凑覆盖率而写的无业务含义测试必须标记为阻塞性问题。
>
