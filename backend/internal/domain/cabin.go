...
type CabinSKU struct {
	...
	VoyageID    int64     `gorm:"index"`               // 所属航次 ID
	CabinTypeID int64     `gorm:"index"`               // 所属舱房类型 ID
	Code        string    `gorm:"size:80;uniqueIndex"` // 舱房编号，全局唯一
	Deck        string    `gorm:"size:20"`             // 所在甲板层
	Area        float64   // 舱房面积（平方米）
	MaxGuests   int       // 最大入住人数
