...
const (
	StaffRoleSuperAdmin = "super_admin"
	StaffRoleOperator   = "operator"
	StaffRoleFinance    = "finance"
	StaffRoleSupport    = "support"
)

var validStaffRoles = []string{StaffRoleSuperAdmin, StaffRoleOperator, StaffRoleFinance, StaffRoleSupport}

func IsValidStaffRole(role string) bool {
	for _, r := range validStaffRoles {
		if r == role {
			return true
		}
	}
...
type Staff struct {
	...
	Role         string     `gorm:"size:20;default:operator"` // 角色：super_admin/operator/finance/support
