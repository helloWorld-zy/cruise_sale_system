...
onMounted(async () => {
  await Promise.all([loadDefaultCruise(), loadContentTemplates()])
})
</script>
  ...
  <div class="admin-page">
    ...
    <AdminFormCard>
      ...
      <section style="border:1px solid #e5e7eb;border-radius:12px;padding:12px;display:grid;gap:10px;">
        <strong>费用说明</strong>
        <select v-model.number="form.fee_note_template_id" data-test="fee-note-template-select" :disabled="loading" @change="onFeeNoteTemplateChange">
        ...
        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
          ...
          <button type="button" data-test="copy-fee-note-to-snapshot" :disabled="loading || !form.fee_note_template_id" @click="copyFeeNoteToSnapshot">复制为航次专属内容</button>
        </div>
        <div v-if="form.fee_note_mode === 'snapshot'" style="display:grid;gap:12px;">
          <div style="border:1px solid #e5e7eb;border-radius:10px;padding:10px;display:grid;gap:8px;">
            <div style="display:flex;justify-content:space-between;align-items:center;">
              <strong>费用包含</strong>
              <button type="button" :disabled="loading" @click="addFeeNoteLine('included')">新增条目</button>
            </div>
            <div v-for="(item, index) in form.fee_note_content.included" :key="`fee-included-${index}`" style="display:flex;gap:8px;align-items:flex-start;">
              <input v-model="item.text" :data-test="`fee-note-included-${index}`" placeholder="费用包含条目" :disabled="loading" style="flex:1;" />
              <button type="button" :disabled="loading" @click="removeFeeNoteLine('included', index)">删除</button>
            </div>
          </div>
          <div style="border:1px solid #e5e7eb;border-radius:10px;padding:10px;display:grid;gap:8px;">
            <div style="display:flex;justify-content:space-between;align-items:center;">
              <strong>费用不包含</strong>
              <button type="button" :disabled="loading" @click="addFeeNoteLine('excluded')">新增条目</button>
            </div>
            <div v-for="(item, index) in form.fee_note_content.excluded" :key="`fee-excluded-${index}`" style="display:flex;gap:8px;align-items:flex-start;">
              <input v-model="item.text" :data-test="`fee-note-excluded-${index}`" placeholder="费用不包含条目" :disabled="loading" style="flex:1;" />
              <button type="button" :disabled="loading" @click="removeFeeNoteLine('excluded', index)">删除</button>
            </div>
          </div>
        </div>
      </section>
      ...
      <section style="border:1px solid #e5e7eb;border-radius:12px;padding:12px;display:grid;gap:10px;">
        <strong>预订须知</strong>
        <select v-model.number="form.booking_notice_template_id" data-test="booking-notice-template-select" :disabled="loading" @change="onBookingNoticeTemplateChange">
        ...
        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
          ...
          <button type="button" data-test="copy-booking-notice-to-snapshot" :disabled="loading || !form.booking_notice_template_id" @click="copyBookingNoticeToSnapshot">复制为航次专属内容</button>
        </div>
          ...
          <section v-for="(section, sectionIndex) in form.booking_notice_content.sections" :key="`booking-section-${sectionIndex}`" style="border:1px solid #e5e7eb;border-radius:10px;padding:10px;display:grid;gap:8px;">
            <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;">
              ...
              <div style="display:flex;gap:8px;">
                <button type="button" :disabled="loading" @click="addBookingNoticeItem(sectionIndex)">新增条目</button>
                <button type="button" :disabled="loading" @click="removeBookingNoticeSection(sectionIndex)">删除分组</button>
              </div>
            </div>
            <div style="display:flex;gap:8px;flex-wrap:wrap;">
              <input v-model="section.title" :data-test="`booking-notice-section-title-${sectionIndex}`" placeholder="分组标题" :disabled="loading" style="flex:1;min-width:180px;" />
              <input v-model="section.key" :data-test="`booking-notice-section-key-${sectionIndex}`" placeholder="分组 key" :disabled="loading" style="flex:1;min-width:180px;" />
            </div>
            <div v-for="(item, itemIndex) in section.items" :key="`booking-item-${sectionIndex}-${itemIndex}`" style="display:flex;gap:8px;align-items:flex-start;">
              <textarea v-model="item.text" :data-test="sectionIndex === 0 && itemIndex === 0 ? 'booking-notice-text-0' : `booking-notice-text-${sectionIndex}-${itemIndex}`" rows="4" placeholder="预订须知正文" :disabled="loading" style="flex:1;" />
              <button type="button" :disabled="loading" @click="removeBookingNoticeItem(sectionIndex, itemIndex)">删除</button>
            </div>
          </section>
          <button type="button" :disabled="loading" @click="addBookingNoticeSection">新增须知分组</button>
        </div>
      </section>
